...
// Locked motion timings for Today and the dock (asserted by unit + e2e specs so
// they can't drift by a stray edit). `press` lives in CSS (.hd-press-deep,
// 160ms); `page` is the section settle (.hd-home-rise); `nav` is the dock
// sinking out of focused work and lifting back; `tab` is the selected
// destination's tint and ink — faster than `nav` because nothing moves, it is a
// colour change and should feel immediate; `reduced` is the ceiling any
// reduced-motion state change may take.
