...
export default function InteractiveChatReader(props) {
  ...
  return (
    <div style={{ ...readerShell(isMobile), background: skin.bg, color: '#111', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      {c.done && <FinishOverlay story={story} accent={accent} onBack={onBack} note={gateCount ? `You replied ${firstTry}/${gateCount} on the first try` : null} core={c} onPractice={props.onPractice} nextChapter={props.nextChapter} onNextChapter={props.onNextStory} onStudy={props.onStudy} />}
    </div>
  )
}
