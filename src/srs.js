// Anki-style scheduler: New → Learning → Review
// grade: 0 = Again, 1 = Hard, 2 = Good, 3 = Easy

const LEARNING_STEPS = [1, 10]      // minutes between learning steps
const HARD_LEARN_MIN = 5            // Hard during learning
const GRADUATING_INTERVAL = 1       // days, when Good graduates a card
const EASY_INTERVAL = 4             // days, when Easy graduates a card
const REVIEW_AGAIN_MIN = 10         // minutes, when a review card lapses
const MIN_EASE = 1.3
...
function mkLearn(step, ease, overrideMin) {
  ...
  return {
    ...
    interval: 0, next_review: minutesFromNow(min), is_easy: false, learned: false,
  }
}
function mkReview(interval, ease, easy) {
  return {
    state: 'review', learning_step: 0, ease_factor: ease,
    interval, next_review: daysFromNow(interval), is_easy: easy, learned: true,
  }
}

// Core: returns { updates, stay, gap, label } for a given card + grade
function computeOutcome(card, grade) {
  ...
  if (grade === 1) {
    ...
    const iv = Math.max(1, Math.round((interval || 1) * 1.2))
    return { updates: mkReview(iv, e, false), stay: false, label: fmtDay(iv) }
  }
  if (grade === 2) {
    const iv = Math.max(1, Math.round((interval || 1) * ease))
    return { updates: mkReview(iv, ease, false), stay: false, label: fmtDay(iv) }
  }
  const e = ease + 0.15
  const iv = Math.max(1, Math.round((interval || 1) * e * 1.3))
...
// Returns { 0: label, 1: label, 2: label, 3: label } for the four buttons
