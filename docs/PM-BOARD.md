...
| HD-P6 | Home & flashcards | in progress | Phone-fit grading and single-RPC grades had already shipped. 2026-08-01: Home shows a queue-derived "~N min" session estimate (`sessionEstimate.js`, documented per-card costs — never a fixed marketing number). 2026-08-05: grade explanations shipped (`gradeHelp.js` + specs — hover `title` per grade button, one "?" InfoTip panel for touch; copy guarded by a no-guilt-words test) and audio failure states made recoverable (AudioButton error state and the Study word-replay "No audio" chip are now retry buttons, not dead chips; a missing clip stays inert). Open: interval-choice investigation. |
...
| HD-P13 | Legal, privacy, a11y, mobile, perf | in progress | 2026-08-01: public `/privacy`, `/terms`, `/support`, `/methodology` pages shipped (reachable signed-out, linked from Landing footer + signup). Written from actual behavior (browser speech recognition — no audio recorded/stored; Analyze-text on-device; first-party analytics; CC-CEDICT/Tatoeba attributions). **Drafts — owner must review before treating as final** (visible beta note on the legal pages). Monitoring shipped end-to-end: unhandled exceptions/rejections/render crashes → capped, privacy-safe `client_error` analytics events (`errorMonitor.js`), now surfaced on `/dashboard` via `admin_client_errors` (applied; staff included by design). 2026-08-05 a11y pass: ChatMission got the focus trap (shared `trapDialogFocus` + `tabIndex={-1}` root); `languageTheme.js` gained a per-language BCP-47 `lang` field and the flashcard word/example sentences + ChatMission text are now `lang`-tagged (WCAG 3.1.2 — SRs were reading hanzi with an English voice on the most-used screen); verified the flashcard reveal surface was already fully focusable (shipped 2026-07-30, roadmap line was stale — corrected). Reduced-motion and dialog traps in the readers were already covered. Open: mobile/perf passes; axe-in-e2e is a follow-up (new dependency). |
| HD-P14 | Automated QA & release gate | in progress | CI (lint/test/build) + Playwright e2e exist; `tests/e2e/trust-pages.spec.js` added 2026-08-01 (4 passing). Published-content validation shipped: `check-published-stories.mjs` + content-utils task `check-published` (runs where the secrets live). First prod run: **0 errors**, 158 warnings — the audio-pending HSK 4–6 stories plus FOUR series with held-chapter gaps: 今天唱歌 (2,4 held), L2 一只跑得很快的兔子 (6 held), L3 田里的田螺 (6–12 held), L3 老王的眼镜 (7–12 held). The L3 serials are missing most of a season each — worth an editorial `publish-held` pass. 2026-08-03: **both remaining pieces shipped** — visual regression (`tests/e2e/visual.spec.js`: landing desktop/mobile, /privacy, stories shelf desktop/mobile; baselines are CI-rendered via the new `visual-baseline.yml` workflow and committed, never captured locally) and the release gate itself (`docs/RELEASE-CHECKLIST.md`: automated/data/content/docs gates per PR, post-merge watch, wider-push list). The shelf shot serves one readable story so the per-day featured pick can't shift the baseline; date-rendering screens are deliberately excluded. Bonus find from the first capture: Landing still claimed a "60-second test" after HD-P11's honesty fix — corrected to the ~3-minute framing. |
