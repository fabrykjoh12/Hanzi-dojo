...
| HD-P5 | Navigation, loading, learner shell | **complete** | 2026-08-07: full screen-by-screen audit + 12 fixes. Shell: failed bootstrap → retry screen (never Onboarding for an existing account), throws can't strand the splash. Honest error-vs-empty states on Home/Test/YouTube/Writing/Stories; Test back button + dead-end CTAs + crash guard; Settings/LanguageSwitcher failed writes revert and report; dead /story/:id links resolve. Audited clean: Practice, Dictionary, Dashboard, Study, Grammar, Auth, drills, public pages. |
