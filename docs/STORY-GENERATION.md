...
- **No candidate has been written by a model yet.** The first real pilot
  ([run 33418327980](https://github.com/fabrykjoh12/Hanzi-dojo/actions/runs/33418327980),
  batch `fab9-pilot-1`) ran the whole pipeline against production — 951 in-band
  vocabulary rows, 204 published stories, two manifests built and committed —
  and reached no model: Gemini returned 429 (credits still depleted), the chain
  failed over as designed, and the Groq standby answered **404, its model id is
  decommissioned**. Both are recorded in `docs/BACKLOG.md`. A 429 or a 404 is
  evidence about infrastructure, never about a model or a story.
- **The manifests inherit the vocabulary's gloss defects.** The pilot's first
  manifest asked for 被 *"quilt; to cover (with)"* and 中 *"China; Chinese"* —
  the known `forms[0]` casualties. The pipeline consumes the curriculum
  authority rather than fixing it, so the ingestion work is now upstream of good
  targeted stories. The validator checks that a required word is present and
