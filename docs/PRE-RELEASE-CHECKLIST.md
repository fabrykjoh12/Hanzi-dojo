...
- [x] **Safe-area basics were already in place** (`viewport-fit=cover` in
      ...
      `env(safe-area-inset-bottom)`). Swept 2026-08-09: every `position:
      fixed` element in `src/` now carries an inset, and `Onboarding` /
      `FirstMissionWelcome` — which render OUTSIDE the shell's `<main>` and so
      inherited nothing — got their own. Still to confirm on real notched
      hardware (§4).
- [x] **Status-bar theming — DONE 2026-08-09.** `src/statusBar.js`
      (`statusBarStyleFor` + specs) maps the app theme to the plugin's
      inverted `Style` values and `App.jsx` pushes it on every theme change.
      **Style only, deliberately**: `setBackgroundColor()` and
      `setOverlaysWebView()` are unavailable on Android 15+ and this app
      targets SDK 36, so the bar is transparent by definition and the only
      thing to control is whether its icons are light or dark. Capacitor 8's
      `SystemBars` handles the insets (it injects `--safe-area-inset-*` and
      pads the container on older WebViews), so `env(safe-area-inset-*)` is
      correct on both platforms with no Android-specific hack.
- [x] **The theme is correct before first paint — DONE 2026-08-09.** `App.jsx`
      used to start at `light` and only learn the truth from the server
      profile, which is a full-screen white flash on every dark-mode cold
      start. `src/themeBoot.js` (+ specs) decides from a device-remembered
      choice, else `prefers-color-scheme`; an inline script in `index.html`
      applies it before the stylesheet paints, and Android has day/night
      `splash_background` colours to match.
- [x] **Native splash lifecycle — DONE 2026-08-09.** `@capacitor/splash-screen`
      with `launchAutoHide: false`, handed over to the web overlay two frames
      after it paints (`src/nativeSplash.js`), so there is no white gap between
      the OS launch image and React's first paint. `SplashIntro` no longer runs
      on a fixed 1,390 ms timer: `splashShouldExit()` waits for the animation
      AND app readiness (`src/appReady.js`), capped at 6 s so a dead network
      cannot trap anyone on a logo. Backstop timer in `main.jsx`, outside
      React, so a render failure can never leave the splash up forever.
- [x] **Fonts are bundled — DONE 2026-08-09.** The render-blocking
      `fonts.googleapis.com` stylesheet is gone: Inter + Poppins (Latin,
      Cyrillic) and Noto Sans SC as one *variable* face ship in the bundle
      (~4.8 MB of woff2 across 112 files; the variable face is 4.6 MB for the
      whole 100–900 axis versus ~15 MB for the four static weights).
      `src/notoSansSC.css` is generated by
      `scripts/build-noto-sans-sc-css.mjs` so the family keeps the name the
      app already uses in ~30 places.
- [x] **Real haptics — DONE 2026-08-09.** `@capacitor/haptics` behind the
      existing `src/haptics.js` API, which was `navigator.vibrate` — a no-op in
      WKWebView, so iPhones had none. Three call sites only: card grade
      (`Study.jsx`), tab change (`MobileNav.jsx`), session complete
      (`SessionRecap.jsx`).
- [x] **iOS audio session — DONE 2026-08-09.** `AppDelegate.swift` sets
      `AVAudioSession` category `.playback`, so pronunciation is audible with
      the ring/silent switch on. WKWebView's default `.ambient` meant a phone
      on silent played nothing, with no indication why. No options and no
      background mode: see the comment in the file for why `.duckOthers` is
      wrong without a deactivation lifecycle.
- [x] **Portrait on both platforms — DONE 2026-08-09.** iPhone was landscape-
      capable while Android was locked; turning an iPhone sideways crossed the
      768px breakpoint and rendered the desktop *website*, dropping the top
      safe-area inset with it. `Info.plist` is portrait-only for iPhone (iPad
      keeps all orientations — the app is still `TARGETED_DEVICE_FAMILY`
      "1,2"), and `shellIsMobile()` in `useIsMobile.js` (+ specs) pins the app
      shell inside the native wrapper regardless of viewport width.
