...
## Database
- [ ] **NOT APPLIED — `20260906120000_grade_card_rejects_stale_replay.sql` (FAB-28 finding 3).** `grade_card` applies `p_updates` as absolute values under `coalesce(...)` with no ordering guard, so a stale offline grade overwrites a newer one and **`reps` can go down**. Sequence, all ordinary use: Monday the phone grades offline (`reps 4 → 5`) and the op sits in the outbox; Tuesday the web grades the same card (`reps 5 → 6`); Wednesday the phone reconnects and replays Monday's op, putting the row back to `reps 5` with Monday's `stability` and `due_at`. The `client_op_id` de-dupe does not help — a stale op is not a replay, it has never been applied. The migration leaves the card's scheduler columns alone when the incoming `last_review` is strictly older than the row's, on **both** write paths, and deliberately still writes the review log and the daily activity (the grade happened; it is the scheduling that is superseded).

  **Apply before merging the PR, or immediately after.** The app does not depend on it — it is a pure tightening of an RPC that already exists, so there is no window where the code expects behaviour the database lacks, in either order. But every day it is unapplied is a day the overwrite is live.

  **It has NOT been executed anywhere.** This repository has no Postgres in its test environment, so `gradeCardStaleReplay.test.mjs` asserts only that the guard is present, that both write paths carry it, that the log and activity writes stayed outside the stale branch, and — the one worth having — that every executable line of the previous 263-line definition still appears in the new one, so nothing was dropped while transforming it. Run it against staging before production.
