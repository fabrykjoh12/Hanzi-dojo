...
## Database
- [ ] **NOT APPLIED — `20260907000000_revoke_pg_net_from_clients.sql` (FAB-26 finding 1).** `pg_net`'s twelve `net.*` functions carry Postgres's default function ACL, which grants EXECUTE to **PUBLIC** — so `anon` and `authenticated` both have it. Verified live: `has_schema_privilege('anon','net','USAGE')` and `has_function_privilege('anon', net.http_post, 'EXECUTE')` are both **true**, and `proacl` on every `net.*` function is NULL (the default). Anyone who reaches one makes *the database* issue arbitrary outbound HTTP — an SSRF primitive inside Supabase's network. The only thing stopping it today is that PostgREST does not expose the `net` schema, which is a **config value, not a grant**.

  **Zero risk to apply.** `pg_net` is completely unused: `20260825120000` removed its only `public` consumer, no trigger on `feedback` remains, and no `public` function references `net.http`. The migration revokes from PUBLIC **first** — revoking from the two named roles alone would leave the inherited PUBLIC grant intact and be a no-op that looked like a fix.
...
- [ ] **NOT APPLIED — `20260907001000_scope_story_children_to_published.sql` (FAB-26 finding 2).** `stories` is correctly scoped to `is_published`; three sibling tables never joined back to it. `story_utterances` and `story_questions` are `USING (true)` for authenticated (115 utterances and 45 questions of unpublished stories, `correct_index` included), and `tts_audio` is readable by **anon** at `status = 'ready'` — so an unauthenticated caller holding the publishable key reads the text of unreleased chapters via `source_text`. 33 of 295 stories are unpublished. An embargo boundary, not a privacy one, and it is live.
  ...
  **Check the reader flows on staging before production.** The analysis says nothing breaks: every client read of the child tables is `.eq('story_id', story.id)` where the story already came from an `is_published = true` query, `admins can read all tts_audio` is a separate policy and is untouched, and `src/tts/**` runs under the service key which bypasses RLS. Vocabulary audio is explicitly excluded from the scoping (`source_type <> 'story_utterance'`), so the anonymous public surface keeps its pronunciation audio. But this changes READ policies on the content path, and none of it was executed here.

  **Neither migration has been run anywhere.** `anonSurfaceMigrations.test.mjs` is structural and says so: it holds the first to revoking from PUBLIC (the detail that decides whether it does anything) and the second to keeping vocabulary audio readable and the admin policy untouched (the two ways it could break something). Both mutation-tested. The rest of FAB-26's findings still need their own migrations — see the Linear issue.
