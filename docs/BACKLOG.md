...
- [ ] 🟡 **Existing `reps` / `review_logs` drift from the old level-test write, unrepaired.** Before PR #240 the level test wrote `reps`, `stability` and `last_review` straight to `cards` with no `review_logs` row. Production carries the result: **61 cards with `reps >= 1` and no log at all, and 104 where `reps` exceeds the log count** (measured 2026-09-07). New drift is stopped — every wrong answer now goes through `grade_card`, which writes the row and the log in one statement — but nothing repairs the existing rows, and they have no owner.

  It is not cosmetic: `reps` is §7.3b's "a human graded this word", and `isLearned`, `isMastered` and the level-test gate all rest on it, so those cards read as better known than the history supports. The repair pattern already exists in this repo — `src/migration/legacyClaim*` did the same job for the prior-knowledge rows. Decide whether to backfill the missing logs (they cannot be reconstructed — the grade and the date are gone) or to leave `reps` as the record and note the gap.

- [ ] 🔵 **`review_logs` has no source tag, so one number counts two different things.** A level-test wrong answer and a study review are indistinguishable in the table, and Profile's *Review accuracy* reads both. Level tests write only their WRONG answers (a correct one reschedules nothing), so a test attempt can only push retention down and that day's review count up — up to 30 rows of `grade = 0` from one attempt. `docs/METRICS.md` documents the asymmetry. Closing it means a `source` column on `review_logs` and a metric split; dropping the log instead is not an option, because that is exactly the `reps`-without-history defect above.
