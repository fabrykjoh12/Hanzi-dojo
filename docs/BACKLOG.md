...
- [ ] **FAB-26 findings 3 and 7 — migrations written, NOT applied** (`20260907010000_cap_dict_add_to_deck.sql`, `20260907011000_revoke_anon_execute_on_private_rpcs.sql`, PR #246's sibling branch). Apply order does not matter; neither depends on the other or on the two anon-surface migrations. Before applying: the revoke migration is the one to watch on staging — sign out completely and check `/read/<id>` and `/how-much-can-you-read` still load, because those are the only two RPCs that keep their `anon` grant. The cap migration builds a partial unique index on `vocabulary`; 6,583 rows, so it is instant, but it will fail to build if a duplicate active level-NULL row has appeared since (0 as of 2026-09-07).
- [ ] **A rate limit is not provenance: `vocabulary` cannot say who added a dictionary row.** `dict_add_to_deck` is capped at 200 adds per caller per 24h, which bounds the pollution rate but not its total, and §7.1 forbids deleting vocabulary — so cleanup after an abusive account is still "find the rows by hand and set `is_active = false`". The clean fix is attribution, and the reason it is not in that migration: a `created_by` column on `vocabulary` would publish "user X added word Y" to every reader of a world-readable table. Doing it properly means a separate provenance table with its own RLS, which is a schema decision rather than a patch.
