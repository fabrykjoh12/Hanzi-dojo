...
- [ ] 🟠 **`20260907030000_repair_reading_plain_drift.sql` — COMMITTED, NOT APPLIED.** FAB-36. Repairs ten `reading_plain` values left behind when `20260724120000_fix_hsk3_6_readings.sql` (applied 2026-08-03, "All 54 rows corrected in prod" per the entry further down this file) corrected `reading` on 54 rows. `reading_plain` is an **answer key** — the flashcard's typed mode (`src/typedAnswer.js`) and the Writing screen (`src/writingMatch.js`) both grade against it — so eight of the ten currently accept the *wrong* pinyin: typing **"han"** for 厂, "yan" for 广, "dui" for 追, "yao" for 约, "ge" for 合, "juan" for 圈, "pan" for 胖, "zang" for 藏.
...
- [ ] **Reset races an in-flight outbox flush.** `dropQueuedWritesForTrack()` (PR #241) deletes the track's queued writes after the reset RPC returns, but it is not a lock: `flushOutbox()` sets a module-level `flushing` flag that nothing outside `syncQueue.js` can see or wait on. A flush that started before the reset can replay an op after the DELETE and before the drop, which resurrects exactly what the drop exists to prevent. The window is one replay round-trip wide and needs a real flush in progress at the moment of a reset, so it is narrow — but it is the same failure, and closing it means an actual queue lock (or a generation counter the flush re-checks per op), not another call site.
- [ ] **`ROADMAP.md` items are cut at their first em-dash before Discord sees them.** `.github/scripts/roadmap-render.mjs` does `item.replace(/ — [\s\S]*$/, '')` and explains it as "this is why ROADMAP.md items are written `**Title.** — description`". The file has not been written that way for a long time: its items are prose with em-dashes mid-sentence, so most are cut somewhere arbitrary. Harmless as truncation, actively wrong when the first half describes a bug and the second half describes the fix — the pinned public message then announces the bug and stops. Two ways out, neither taken: keep the whole item (the cap is on item count, not length), or cut on a marker that cannot occur mid-sentence. Until then, check what an item publishes as before merging it.
- [ ] **An untagged queued write cannot be attributed to a language, so resetting the track you are ON still drops it.** `queuedOpBelongsToTrack` drops a `grade`/`storyRead`/`storyClaim` op carrying no `language`/`system` tag when the track being cleaned is the learner's active one: it cannot tell whose the op is, and a silently half-undone reset is the worse failure there. Untagged is what the **current production build** queues, so until the tagging change has shipped and every queue has drained, resetting your current language can discard an unsynced grade that came from another one. This is why the reset dialog now promises nothing about the other languages.

  **Narrowed, not closed.** `shouldDropUntaggedOps` keeps untagged ops whenever the track being cleaned is *not* the active one — resetting a track you are not studying, and the remove-language panel, which by construction only ever targets a track nobody can currently start. There the trade inverts: nothing of that track's was deleted, so an untagged op resurrects nothing, and dropping it destroys a write that would have replayed correctly. That was a strict regression on the behaviour before the drop existed, and it is gone.

  **The exact fix, when it is worth doing:** an untagged op still carries the id of the thing it wrote about — `vocabId` for a grade, `storyId` for a story read — and both `vocabulary` and `stories` carry `language`/`system`. One `select id … in (…)` per reset resolves them exactly and the trade disappears. Not done here because it puts a network round-trip in the middle of a destructive path for a case that needs an old build, an offline session and a second active track at once; once the tagged build has been out a while, the case ages out instead.
- [ ] **Two admin paths delete learning rows without draining the queue that wrote them.** `src/Dev.jsx` deletes `cards` directly and, separately, every `story_reads` row for the account. Neither goes through `reset_language_progress`, so `resetCallSites.test.js` — which finds reset paths by their RPC call — does not see them, and neither drops the queued writes those rows belong to: the same stranding `dropQueuedWritesForTrack` exists to prevent, with the same poison-pill ending (a replayed grade for a deleted card raises `'Card not found'` forever). Admin-only and pre-existing, and the raw `cards` delete also sits oddly against §7.2. Left alone here rather than widened into this change; the fix is either routing both through the RPC or giving each the same drop.

- [ ] **A held foreign-account op never expires.** `flushOutbox` now skips a queued op that names a different account rather than replaying it as the signed-in one, and `pendingWrites(userId)` stops counting it — so it is invisible and harmless while that account is away. It is also permanent: nothing removes it if that account never signs in on this device again, and `outboxClear()` still runs only on account deletion. Bounded by the outbox's own size (a handful of rows per offline session), so it is storage hygiene rather than a bug, and it belongs with the same durable-write policy question as the missing attempt counter.
- [ ] **A full reset deletes all `daily_activity`, and a correctly-kept op re-creates part of it.** `reset_language_progress` with `p_reset_account_history = true` clears the user's whole `daily_activity` (`20260809090000`), which is account-wide rather than per-track. A queued grade for *another* language is correctly kept by `queuedOpBelongsToTrack`, and `reconcile()` folds its day counts back in on the next flush — so a day row reappears for a day the reset had cleared. Keeping the op is right (it is another track's unsynced work) and the effect is cosmetic: the study calendar shows a day it had forgotten, with counts that really happened. Fixing it properly means the reset and the outbox agreeing on what "account history" covers, which is a bigger question than the drop.
- [ ] **The offline caches survive a reset, so an offline app still shows pre-reset progress.** `data.js` caches the card list under `cacheSet(key, …)` and `Stories.jsx` caches a whole shelf snapshot (`cardsData`, `readsData`, `unlocksData`) — both read only when the fetch fails. Nothing clears either on reset, so a learner who resets and then loses connection sees their old cards, read marks and unlocks come back from IndexedDB. **It is not display-only, which an earlier version of this entry said.** `sessionPrep` builds the study session from `getTrackCards`, so offline that session is built from the cached deck — and grading one of those cards enqueues a grade carrying the `cardId` of a row the reset deleted. On replay that takes `grade_card`'s UPDATE branch, which filters `c.user_id = auth.uid()` and raises `'Card not found'`: the permanent poison pill, in a queue with no attempt counter. So `ROADMAP.md`'s "no longer hands your next study session a set of deleted cards" is true of the in-memory prepared slot and true online, and this is the case it does not cover. The fix is cache invalidation on reset, alongside the queue drop; `clearDownloads()` deliberately leaves the outbox alone and would need the same care in reverse.
