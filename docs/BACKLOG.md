...
## Learning quality
- [ ] **HSK 3–6 wrong readings — MIGRATION WRITTEN, NOT APPLIED.** 37 of the ~1,870 HSK 3–6 words shipped with a wrong `reading`. This is not cosmetic: Chinese audio is synthesised *from* the reading — `generate-audio.mjs` speaks the hanzi with pronunciation pinned to `v.reading` via SSML — so each of these words currently **shows** the wrong pinyin and **says** it out loud. Three classes: CC-CEDICT's ASCII `u:` for ü leaked in verbatim (忽略 `hū lu:è`, 战略, 策略); a rare reading was picked over the everyday one (厂 `hǎn` not `chǎng`, 转 `zhuǎi` not `zhuǎn`, 追 `duī` not `zhuī`, 广 `yǎn` not `guǎng`, 藏 `Zàng` not `cáng`, 作 `zuō`, 抢 `qiāng`, 圈 `juān`, 胖 `pán`, 合 `gě`, 约 `yāo`, plus 匹 `pī`→`pǐ`); and a proper-noun capital on ordinary words (成功 `Chéng gōng`, 和平, 美元, 网络, 资源, 大众, 通道, 时代, 现代, 将军, and 12 single chars). Found by diffing `vocabulary.reading` against the CC-CEDICT `dict_entries` already in the project; all 37 replacements are themselves CC-CEDICT-attested, and none of the 37 is in any learner's deck yet (0 `cards` rows), so fixing now costs nothing. Fix: apply `supabase/migrations/20260724120000_fix_hsk3_6_readings.sql` (idempotent — matches on the known-bad value, so a re-run or a hand-fix is never clobbered), **then re-run Actions → task `audio-hsk3-6`** or the clips stay wrong. ⚠️ Do *not* null `audio_path` to force that: the generator's work list is `vocab.filter(v => v.audio_path)` with `upsert: true`, so clearing the path *excludes* a word from regeneration. Deliberately left alone: genuine proper nouns (上帝, 圣诞节, 国会, 佛) and ~14 words where both readings are defensible in context (待 dāi/dài, 答 dā/dá, 结 jiē/jié, 泡, 档, 扇, 尽, 切, 挨, 晕, 杆, 踏, 码头, 眼里) — those want a native-speaker call, not a blind edit.
