...
- [ ] 🟠 **`20260907030000_repair_reading_plain_drift.sql` — COMMITTED, NOT APPLIED.** FAB-36. Repairs ten `reading_plain` values left behind when `20260803` corrected `reading` on 54 rows. `reading_plain` is an **answer key** — `checkAnswer` (`src/testLogic.js`) accepts it, and the level test requires 100% to unlock the next level — so eight of the ten currently accept the *wrong* pinyin: typing **"han"** for 厂, "yan" for 广, "dui" for 追, "yao" for 约, "ge" for 合, "juan" for 圈, "pan" for 胖, "zang" for 藏.
  ...
  **Order does not matter**; it depends on no other pending migration and no app change. Idempotent and self-limiting: it updates only rows that disagree, so re-running touches none. Verified against live data before writing — applied today it changes exactly ten rows.

  **The column has two conventions, and a first draft would have destroyed one.** Recomputing a canonical "lowercase, no spaces, no apostrophes" form would have rewritten **21** rows, eleven of them correct: the hand-curated HSK 1 rows keep spaces (你好 "ni hao", 打电话 "da dianhua"), capitals (中国 "Zhongguo", 汉语 "Hanyu") or an apostrophe (女儿 "nu'er"), while the bulk-imported rows carry the squashed form. Both are valid answer keys because `lenientPinyin` strips all three from both sides. The comparison ignores exactly what `lenientPinyin` ignores, and the value written preserves the row's own shape.
