...
## Database — needs applying
- [ ] **APPLY `20260728210000_fix_language_reset_missing_writing_stats.sql`.** "Reset HSK 3.0 progress" on Profile fails outright with `relation "public.writing_stats" does not exist`, so a language's progress cannot be cleared at all. Root cause is the §10 classic: `20260605224500_add_writing_stats.sql` is in the repo but was **never applied to this project**, while the reset RPC that deletes from that table was. It costs more than the reset — `src/Writing.jsx` reads and upserts `writing_stats` on every writing answer, so writing practice has been discarding its results the whole time. The new migration creates the table idempotently AND guards the RPC's delete with `to_regclass`, so a missing optional table can never abort a reset again. **Apply via the Supabase MCP `apply_migration` or the dashboard SQL editor** — the migration is written and committed, only the apply is outstanding. Verify after: run the reset from Profile, and confirm `select count(*) from writing_stats` resolves.
