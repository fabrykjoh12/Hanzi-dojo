...
## Database
- [ ] **NOT APPLIED — `20260906120000_grade_card_rejects_stale_replay.sql` (FAB-28 finding 3).** `grade_card` applies `p_updates` as absolute values under `coalesce(...)` with no ordering guard, so a stale offline grade overwrites a newer one and **`reps` can go down**. Sequence, all ordinary use: Monday the phone grades offline (`reps 4 → 5`) and the op sits in the outbox; Tuesday the web grades the same card (`reps 5 → 6`); Wednesday the phone reconnects and replays Monday's op, putting the row back to `reps 5` with Monday's `stability` and `due_at`. The `client_op_id` de-dupe does not help — a stale op is not a replay, it has never been applied.

  **The ordering key is `reps`, not `last_review`, and that matters.** A first draft compared `last_review`, which is stamped by the *device* clock — and `20260822160000` already ruled that two timestamps from two clock domains must not be ordered by the database. It fails both ways: a slow phone clock makes the guard discard a genuinely newer grade (while still writing its review-log row, so the log count then exceeds `reps` — the same disagreement, inverted), and a fast one lets the stale write land anyway. `reps` is not a clock: `srs.schedule()` increments the row the client read, so a newer grade always carries `existing + 1`. The guard also lives inside the `UPDATE`'s own `WHERE` and the `ON CONFLICT DO UPDATE`'s `WHERE`, not in a separate `SELECT` first, so a grade committing concurrently cannot slip between a check and a write.

  **Apply before merging the PR, or immediately after.** The app depends on nothing it adds — it is a pure tightening of an existing RPC — so there is no window where the code expects behaviour the database lacks, in either order. But every day it is unapplied is a day the overwrite is live.

  **It has NOT been executed anywhere.** There is no Postgres in this repository's test environment. `gradeCardStaleReplay.test.mjs` is structural and says so in its own header: it asserts the guard keys on `reps` and on no timestamp, that it sits inside both write statements and reads the raw incoming value on both (not `excluded.reps`, which the insert coalesces to 0 and which would make the fail-open limb dead on that path), that a refused grade writes **neither** a review log nor a day count, that the client reads the `stale` flag, and — the one worth having — that every executable line of the previous 263-line definition still appears the same number of times, with every intended addition and removal enumerated. That last check is a *multiset* comparison in both directions; the first version compared sets, and then only reported LOST lines, so a stray `delete from public.review_logs` or a widened grant would have passed green. **Run it against staging before production.**

  **What it does NOT close.** Four client paths write `cards` scheduler columns without going through `grade_card` — `src/Study.jsx`'s undo (which writes the client's pre-grade snapshot straight over whatever the server holds), `src/Study.jsx`'s `resetCard`, `src/Test.jsx`, and `src/CreativeMode.jsx`. "`reps` can go down" is therefore narrowed by this migration, not eliminated. Separate work.

  `resetCard` interacts with the guard rather than merely sitting beside it: it writes `reps 0`, so after a reset an outbox op carrying `reps 5` satisfies `5 > 0` and is **admitted as newer**, resurrecting the pre-reset scheduler state with the guard's blessing. PR #241's reset outbox drop is what closes that, by deleting those ops when the reset happens. The guard cannot: `reps` is not monotonic across a reset, and nothing in the row records that one occurred.

  **The guard is only half in the database.** A refused write comes back with `stale: true`, and `gradeCardWrite` surfaces it while keeping `ok: true` — the op is settled and must not be retried, but the caller's local card is behind the server's. Four things in `Study.jsx` had to stop treating it as an applied grade, and each is its own way of undoing the guard:

  - it no longer **reinserts** the card into the queue carrying state the server declined, so the next grade is not computed from a state that exists nowhere;
  - it no longer offers **undo**, which writes the pre-grade snapshot to `cards` with a bare `UPDATE` that `grade_card` never sees — one tap would have restored exactly what the guard refused, offered at the moment the learner is most likely to press it;
  - it no longer commits the day counts. Study sends **absolute** counts (`mode: 'set'`), so committing them would have put the refused card into the number the *next* successful grade writes, undoing `not v_stale` one statement later;
  - it rolls back the **recap tally**, which runs before the write and feeds `qualifiesForReward`.

  The notice has its own channel rather than `setSaveError`. That one renders the fatal "your progress is not being saved … run the migration SQL in your Supabase SQL Editor" banner — false here (progress *is* saving; one superseded answer was not), never cleared because nothing sets it back to null, and it tells a store-app learner to open a database console.

  **An op the server can never apply now leaves the outbox.** Narrowing `isMissingRpc` was right, but on its own it stranded a `Card not found` op forever: it used to classify as a missing RPC, fall to the legacy path, match zero rows without error and be deleted. `isUnreplayable` makes that drop deliberate and narrow — SQLSTATE P0001 with that message only — because otherwise `pendingWrites()` never reaches zero and the sync pill reads "Syncing 1 saved review…" on every mount, forever. A transient failure (serialization, RLS, network) is still left in place; a spec pins both directions.

  **`isMissingRpc` had to be narrowed for any of this to hold.** `grade_card` raises `'Card not found'` for an op naming a row that is gone — an undo of a new card, or a language reset, leaves exactly such an op in the outbox — and `isMissingRpc` matched a bare `'not found'`. That classified a deliberate application error as "the RPC isn't deployed", latched `rpcUnavailable` for the whole page load, and dropped every later grade to the legacy path: a bare `UPDATE` with no guard at all. One deleted card disabled this migration's entire guarantee until reload.

  **Blast radius, before applying.** Nothing here repairs rows already damaged by past stale replays, and the superseded state is not recoverable. This counts the accounts affected:

  ```sql
  select c.user_id, count(*) as cards_with_more_logs_than_reps
  from public.cards c
  where (select count(*) from public.review_logs l where l.card_id = c.id) > c.reps
  group by c.user_id order by 2 desc;
  ```
