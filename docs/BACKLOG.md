...
- [ ] 🔵 **"Add to my due list" is a dead control on a prior-knowledge claim.** `src/Writing.jsx` now correctly refuses to nudge an unverified claim — its `due_at` is the calibration spread date, not a review date — but the button still renders and stays enabled, and clicking it produces no state change and no message. Better than the false green confirmation it used to show, and reachable: `studiedWords` keeps any level vocabulary with a card row, so claimed words do enter writing practice. The fix is not offering the control for a claim (or offering "check this word now" instead, which is what the learner actually wants); that is a UI decision on a screen this change did not otherwise touch.

- [ ] 🔵 **`src/Analyzer.jsx` classifies a prior-knowledge claim as `learning`.** Its card query selects `vocab_id, is_easy, state` and feeds those rows to `calculateStoryReadability` → `wordStatus` → `isPriorKnown` (`storyReading.js`), which reads `prior_known_at` and `reps` — neither of which is fetched, so the predicate is always false and a claim reads as an ordinary started card. Pre-existing and identical on `main`. `knowledgeState.PRIOR_KNOWLEDGE_COLUMNS` now exists for exactly this and does not yet reach here; `Study.jsx` and `Stories.jsx` spell the two columns literally, which works but drifts the same way. Third instance of this defect class in the repo, after `Test.jsx` (#240) and `Writing.jsx` (#243).
