...
- [ ] **35 HSK 6 words still have no example sentence** (down from 335 on 2026-07-28). Levels 3, 4 and 5 are complete; the LLM fill wrote 300 of the 335 before **both** free tiers hit their daily wall — Gemini first, then Groq (`Rate limit reached … tokens per day (TPD): Limit 100000, Used 99085`, `retry-after: 3440`). Nothing to fix: re-run Actions → `examples-fill`, `language: chinese`, `level: 6` after the quotas reset (Groq is hourly-ish from its own reset header, Gemini daily) and the last 35 land in one short run. History below.
