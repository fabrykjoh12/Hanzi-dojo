...
## Admin tooling
- [ ] **⚠️ Dojo HQ — MIGRATION WRITTEN, NOT APPLIED.** `/hq` is now backed by the app's own Supabase project and keyed to the signed-in account (`src/dojoSupabaseClient.js`), replacing the `localStorage` device board that could never be shared. It needs `supabase/migrations/20260727140000_add_dojo_hq.sql` applied — `dojo_items`, `dojo_comments`, `dojo_attachments`, all with RLS where every policy requires `exists (select 1 from profiles where id = auth.uid() and is_admin)`, plus a security-definer `dojo_hq_members()` and a private `dojo-attachments` bucket. Until it is applied the screen says so by name rather than showing an empty board (the standard silent-no-op trap — CLAUDE.md §10). Membership *is* `profiles.is_admin`: every admin shares one board, so a second person needs `is_admin = true` (`/make-admin`). No ordering dependency with the pending reading migrations — different tables.
