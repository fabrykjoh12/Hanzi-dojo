...
- [ ] **An untagged queued write cannot be attributed to a language, so a reset drops it whichever track it came from.** `queuedOpBelongsToTrack` drops a `grade`/`storyRead`/`storyClaim` op carrying no `language`/`system` tag, because it cannot tell whose it is and a silently half-undone reset is the worse failure. Untagged is what the **current production build** queues, so until the tagging change has shipped and every queue has drained, resetting one language can discard another's unsynced grades — and the remove-language panel is worse, since the track being cleaned up is the one being removed, so the loss falls on the learner's *active* track. This is why the reset dialog now promises nothing about the other languages.
