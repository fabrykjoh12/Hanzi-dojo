...
- [x] **Cover art for all 60 new hand-authored chapters — DONE 2026-07-29.** Ten seasons, ten deliberately distinct styles rather than one house look, each picked to suit its story: warm gouache (HSK 1 我做晚饭), ink wash (HSK 1 店外边的椅子), woodblock print in three temperatures for the three legs of the sea strand (cold → night → dawn), two-colour risograph (HSK 2 天天来的那只狗), manhua with screentone (HSK 3 一个字), cinematic noir (HSK 4 五楼的灯), mid-century screenprint (HSK 5 对面的新店), torn-paper collage (HSK 6 大毛是从哪儿来的). Every prompt carried an explicit no-text/no-signage constraint — generated hanzi is always wrong and this is a language-learning app, so a cover must never show a character a learner might read. 16:9 to match the fixed cover slot in `Stories.jsx`.
