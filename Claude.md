...
- **Kana (item 5):** three tabs now — **Learn | Practice | Chart**. Chart = browsable gojūon grid, tap any kana to hear it (`speakKana`, ja-JP TTS). **Learn = guided learn-then-quiz**: one gojūon row per lesson (`LESSONS`), study screen shows the row's hira+kata+romaji (tap to hear) + prev/next lesson browse, then "Quiz these kana" drills just that row (`startLesson`; distractors drawn from `ALL_ROMAJI` so even 3-kana rows work in tap mode), and the recap offers "Next lesson". Practice (row-select quiz) is unchanged.
