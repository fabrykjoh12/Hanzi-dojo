...
- **Dojo HQ is ADMIN-ONLY.** It is internal tooling (workspaces, invite codes, member management, Norwegian UI) and shipped in `PRIMARY_NAV` with no route gate, so every learner saw it and could open it. It now lives in `ADMIN_NAV` (an array, alongside Dashboard) and `App.jsx` gates `view === 'hq'` on `profile.is_admin` the same way the dashboard is gated — hiding a menu entry is not access control. Pinned by `src/navConfig.test.js` and `tests/e2e/hq-gate.spec.js`.
  - ⚠️ `hq.html` is a **separate Vite entry**. It is excluded from the public build (`vite.config.js` adds it only for the Sites target) and Vercel rewrites every path to `index.html`, so `/hq` in production reaches App and hits the gate. On the **dev server** Vite's html-fallback serves `hq.html` at `/hq` directly, bypassing the router — expected, and why the e2e drives the client router instead of a raw URL visit.
...
- **Home screen:** One lit `HeroPanel` about **today's flashcards** — cards waiting, the New/Learning/Due breakdown, the daily goal, and ONE action ("Start reviewing", or "Read a story" once the queue is clear). Beneath it a quiet flat "Then read" hand-off to the day's story (the next step in the loop, deliberately not a rival CTA) and progress toward the next level. The ensō ring was removed (user preference) along with `EnsoRing.jsx`/`enso.js`. Below that, **"Your week"** renders the previously-orphaned `counts.rhythm7` (seven day marks — filled = studied, today ringed until earned) and the `forecast7` load ahead ("about N waiting tomorrow"). Copy is observational by design: **no streak counter, nothing to "keep"** — the return hook is the work waiting, not guilt. `weekdayInitial()` in `studyRhythm.js` parses the date key at NOON local, because bare `new Date('YYYY-MM-DD')` is UTC midnight and would label the strip a day early for every learner west of UTC.
