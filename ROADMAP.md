...
- [ ] **The app got a lot calmer.** Testing the last build on a real phone made one thing clear: the screens were trying too hard. So the whole experience has been redesigned around restraint. Home is now a launcher, not a dashboard — the date, one vermilion panel with today's cards, plain rows for your story and practice, and your level progress; nothing on it is ever locked or greyed out. The bottom navigation is a normal, quiet tab bar (the floating tray with the raised Home bump is gone), and it steps out of the way entirely during a flashcard session. Opening Cards is instant now — the session is prepared while you're still on Home, so there's no loading screen between tapping and seeing your first card. The grade buttons, story screens, practice hub and profile all moved to the same quieter language: text on paper, one accent, surfaces only where you can pick something up. In review on the redesign branch, headed for physical-device QA and TestFlight.
