...
**Just shipped**
- [x] **The story page is a library now — you pick the HSK you want to read, and the shelf is sorted by how much of it you already know.** Reading used to be organised the app's way: three tabs called First Steps, Growing and Fluent, each holding a pile of every level at once, with no way to say "I want to read HSK 2 tonight". Now the first thing on the page is a **rail of levels** — every level you've reached, the one you're on marked with a dot, and the ones above you sitting there dimmed with a count of what's already waiting up there, because seeing that HSK 4 has twelve stories in it is worth more than a locked door. Pick one and its whole shelf is on screen: the series as single stacked covers, the standalone stories, the practice scenarios in their own section underneath. **Every cover now tells you how readable it is** — a thin bar along the foot of the picture and a plain "94% known" on the card, both worked out with exactly the same maths the reader uses, so the number never disagrees with itself. That's what the shelf sorts by. **For you** leads with the series you're part-way through, then everything unread, most-readable first. **Easiest** and **Hardest** do what they say. **Shortest** is there for the evenings you have ten minutes. **In order** is the shelf as it was written. There's a **search box** that looks in titles, English summaries and the Chinese itself, so both "noodle" and 面条 find the same story. And the stories a tier hasn't opened for you yet are no longer invisible: they sit at the bottom under *Not open yet*, dimmed, each one saying how many more learned words open it — nothing is hidden from you, it just isn't your turn yet.
