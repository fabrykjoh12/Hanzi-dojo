...
- [ ] **Home goes back to the classic "Today" layout.** After trying a few redesigns, the front page returns to the version that worked best: one card about today's flashcards — how many are waiting, what the session is made of (new / learning / review, now on phones too), your daily goal, and one Start button — with tonight's story as a quiet "Then read" hand-off underneath, and your week's rhythm plus progress toward the next HSK level in a single calm panel below. Everything newer under the hood stays: the session is still prepared while you look at the screen so Study opens instantly, the story hand-off still knows tonight's real chapter and how much of it you can read, and the screen still settles quietly once everything is done. On a branch, awaiting design sign-off.
