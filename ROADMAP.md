...
**Just shipped**
- [x] **Home now counts every card that's actually waiting for you.** If you'd saved a word from a story or looked one up in the dictionary, that card was due — the review session served it — but Home didn't count it, so the number on the front page promised a shorter session than you got. Home and the session now work from one shared definition of what's due, so what you're told is what you get. Your level progress still counts only the words at your level, which is what that bar has always meant.
- [x] **The internal control room no longer ships inside the app.** Hanzi Dojo has a private tool the maintainers use to run the project. It was never reachable from the menu, but a copy of it was still being packaged into the app itself. It's now excluded from the build outright — the app that goes to the App Store and Play simply doesn't contain it — and an automatic check on every change makes sure it stays that way.
- [x] **Proper credit where the app builds on other people's work.** Hanzi Dojo uses open datasets — CC-CEDICT for dictionary definitions, Tatoeba for example sentences, open stroke-order data, open fonts — and the app now says so properly: each one named, its licence linked, and, for the dictionary, an honest account of what we changed (readings corrected, definitions trimmed for learners, a study order applied) with the adapted data offered back under the same licence. The Terms also stopped overclaiming: we assert what's actually ours, and say plainly that some artwork was made with AI tools rather than pretending otherwise. There's now a licence file and an attribution notice in the open repository too.
...
## ✅ Shipped
- [x] **A calmer, more beautiful Home.** The front page is back to the layout that worked best — and then some. One card carries today's session: how many cards are waiting and what they're made of (new / learning / review), with the whole card as the button, so tapping anywhere starts studying and the panel grows straight into the flashcard. Tonight's story sits underneath as a quiet "Then read" hand-off with its own cover, its level and how much of it you can already read; stories without artwork now get their own drawn cover instead of a placeholder letter. Your week's rhythm and your progress toward the next HSK level share one calm panel below. Behind it all, the screen now has a landscape: a horizon of layered hills that shifts through morning, day, evening and night, and a hand-drawn cinnabar mountain scene inside the session card. Everything still opens instantly, and once the day's work is done the screen tells you what tomorrow holds and goes quiet.
