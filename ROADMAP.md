...
- [ ] **Home + navigation v3 production migration** — the approved Cards → Story → Practice Home and Stories — Home — Practice navigation are locked, and are now integrated with the release-readiness work on one branch. Old Cards/More deep links still work, every daily stage comes from real learning state, and the 320/390/430 light/dark, motion, state-matrix and back-navigation checks pass. Remaining before physical-device QA: the store-candidate gate on CI.
