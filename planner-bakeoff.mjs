...
for (const p of (stored ? [] : planners)) {
  for (let k = 1; k <= perModel; k += 1) {
    ...
    let error = null
    try {
      bp = parseBlueprint(await p.send({ kind: 'shape', prompt, maxTokens: 2600 }))
...
const byModel = {}
for (const c of candidates) {
  const m = byModel[c.model] || (byModel[c.model] = { n: 0, structural: 0, quality: 0, joint: 0, overall: [], buckets: {} })
  m.n += 1
