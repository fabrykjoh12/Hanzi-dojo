...
test.describe('Manga reader', () => {
  ...
  test('opens on the first panel with the episode header', async ({ page }) => {
    ...
    await expect(page.getByLabel('Panel 1 of 8')).toBeVisible();
  ...
  test('a panel is a figure with alt text, and the art is a real image', async ({ page }) => {
    await openEpisode(page);
    const art = page.getByRole('img', { name: /traveller at the foot of a lantern-lit stair/ });
  ...
  test('the choice gates the rest of the episode until it is answered', async ({ page }) => {
    ...
    // The reply becomes a line of the story, and the episode continues.
    await expect(page.getByText('Choose a reply to keep reading.')).toHaveCount(0);
  ...
  test('the header counts panels as you scroll', async ({ page }) => {
    ...
    await page.getByRole('img', { name: /Close-up of 小雨/ })
      .evaluate(el => el.scrollIntoView({ block: 'center' }));
    await expect(page.getByLabel('Panel 3 of 8')).toBeVisible();
  });
  ...
  test('completing the episode shows the seal, the words met, and the hook', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /是，我是新学生。/ }).click();
    // Walk to the end.
    await page.getByRole('img', { name: /walks in through the gate/ }).scrollIntoViewIfNeeded();
    ...
    await expect(done.getByText(/第二话 is being drawn/)).toBeVisible();
