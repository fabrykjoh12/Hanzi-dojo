...
const PANEL_COUNT = NOODLESHOP_MANIFEST.panels.panels.length;
...
test.describe('《一块钱》 vertical slice', () => {
  ...
  test('runs linearly through captions, resume, comprehension, reward, and read state', async ({ page }) => {
    ...
    // Educational text remains live UI below the art: translation and audio
    // are available on the line itself without covering the illustration.
    ...
    const catArt = page.getByRole('img', { name: /cat and the reader’s outstretched hand/ });
    await catArt.scrollIntoViewIfNeeded();
    ...
    // Persist the reading position, then prove a new reader mount resumes at
    // the saved panel instead of starting over.
    ...
    await page.getByRole('img', { name: /neither of them looking at the other/ })
      .evaluate(el => el.scrollIntoView({ block: 'center' }));
