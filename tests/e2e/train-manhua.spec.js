...
test.describe('《末班车》 standalone story', () => {
  ...
  test('appears as a complete HSK 2 work outside every series', async ({ page }) => {
    ...
    const standalone = shelf.locator('section[aria-labelledby="standalone-stories-2"]');
  ...
  test('runs through both choices, resume, questions, reward, and read state', async ({ page }) => {
    test.setTimeout(120000);
    await openStory(page);

    await expect(page.getByText('第一话', { exact: true })).toHaveCount(0);
    await expect(page.getByLabel('Panel 1 of 20')).toBeVisible();
    ...
    await page.getByRole('button', { name: /我们跑过去！/ }).click();
    await expect(page.getByRole('img', { name: /two small figures bursting out of the restaurant doorway/ })).toBeVisible();

    await page.getByRole('img', { name: /wall of legs, bags and umbrellas/ })
      .evaluate(el => el.scrollIntoView({ block: 'center' }));
    await expect(page.getByLabel('Panel 11 of 20')).toBeVisible();
    await page.getByRole('button', { name: 'Back to stories' }).first().click();
    await openStory(page);
    await expect(page.getByLabel('Panel 11 of 20')).toBeVisible();
    await expect(page.getByRole('button', { name: /我们跑过去！/ })).toHaveAttribute('aria-pressed', 'true');

    await page.getByRole('button', { name: /你去那边，我去这边！/ }).click();
    await page.getByRole('img', { name: /dark restaurant window from outside/ }).scrollIntoViewIfNeeded();
